...
const menteeApiController = {
    ...
    signOut : catchAsync( async (req, res, next) => {
        ...
        const foundUser = await Mentee.findOneAndUpdate({ accessToken: token }, { accessToken : null })
    ...
    getParticularCourse : catchAsync( async (req, res, next) => {
        ...
        const foundCourse = await Courses.findById(courseId).populate('videos').populate('Mentor')
    ...
    buyCourse : catchAsync( async (req, res, next) => {
        ...
        const foundMentee = await Mentee.findByIdAndUpdate(MenteeId.id, { $push : { courses : courseId }}, { new : true })
        const foundCourse = await Courses.findByIdAndUpdate(courseId , { $inc : { revenue : req.body.amount}}, { new : true })
        await Mentor.findByIdAndUpdate(foundCourse.Mentor, { $inc : { revenue : req.body.amount}})
    ...
    getUserData : catchAsync( async (req, res, next) => {
        ...
        const foundMentor = await Mentor.findById(userId.id).populate({path : 'courses', model : 'course', populate: { path: 'videos', model : 'videos'}})
        if(!foundMentor && !foundMentee) return next(new AppError('invalid credentials', 400))
        if( foundMentee ){
            return res.json({
                status : 'success',
                user : 'Mentee',
